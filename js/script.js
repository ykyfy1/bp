...
const helpContent = {
    ...
    myWord: {
        header: "新增文字烟花",
        body: "添加自定义文字烟花，输入你想要的文字，点击回车或者任意位置，自动添加成功",
    },
    wordShell: {
        header: "文字烟花",
...
// 处理用户输入的祝福语
const myWordInput = document.querySelector(".my-word");
myWordInput.addEventListener("change", (event) => {
    const newWord = event.target.value.trim();
    if (newWord) {
        updateRandomWords(newWord);
...
function colorSky(speed) {
    ...
    appNodes.canvasContainer.style.backgroundColor = `rgb(${currentSkyColor.r | 0}, ${currentSkyColor.g | 0}, ${
        currentSkyColor.b | 0
    })`;
}
